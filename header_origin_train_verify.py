# header_origin_train_verify_v2.py
import hashlib, json
...
def main():
    ...
    # ---------- 1) Grouped stratified split (zero overlap) ----------
    # Use features-only signature as groups to prevent any overlap
    ...
    print(f"Train/Test overlap by row signature (stratified+grouped): {len(sig_tr & sig_te)}")
    ...
    # Save the grouped-stratified model for deployment
    joblib.dump(model, SAVE_MODEL_PATH)
    print(f"\nSaved model: {SAVE_MODEL_PATH}")
    print(f"Saved columns: {SAVE_COLS_PATH}")

    # ---------- 2) Deterministic hash split (no overlap by construction) ----------
    ...
    print(f"\nTrain/Test overlap by row signature (hash split): {len(sig_tr_h & sig_te_h)}")

    rf_h = RandomForestClassifier(
        n_estimators=250, class_weight="balanced", n_jobs=-1, random_state=RANDOM_STATE
    )
    _model_h, _, y_proba_h = train_and_report(rf_h, X_train_h, y_train_h, X_test_h, y_test_h, split_name="hash_split")
